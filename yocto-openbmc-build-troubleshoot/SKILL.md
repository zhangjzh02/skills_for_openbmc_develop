...
**如果 args 中包含用户原始消息未提及的信息（如机型、recipe），应视为调用方编造，直接忽略并向用户确认，而非采信。**
...
所有 `Shell` 调用必须显式设置 `timeout`，默认策略如下：

| 编译目标 | 默认超时 | 说明 |
|----------|----------|------|
| 单个 recipe / `-c compile` | 3600 秒（1 小时） | 适用于大多数模块级编译 |
| 完整镜像（如 `obmc-phosphor-image`） | 14400 秒（4 小时） | 完整镜像或大量 sstate 缺失时自动延长 |
| 用户明确指定 | 以用户指定为准 | 如用户说"超时设 8 小时"，则优先遵从 |

**超时上限**：工具最大支持 86400 秒（24 小时）。若预估编译时间可能超过默认值，应主动询问用户是否需要调整，或在用户提及"大镜像"/"网络慢"/"sstate 缺失"等场景下自动使用更长超时。
